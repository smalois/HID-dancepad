...
int zeroed_out(int cur_force, int zero_val) {
	...
	return ((float)abs((cur_force - zero_val)) / (float)denominator) * ptof(1024);
}

bool is_pressed(struct panel cur_panel) {
	unsigned long denominator = ptof(1024) - cur_panel.base_force;
	unsigned long nominator = cur_panel.cur_force - cur_panel.base_force;
	double percentage_pressed = nominator / denominator;
